 ...
 * Also ensures UIBackgroundModes includes remote-notification (required for APNs).
...
function patchVpIosInfoPlist(plistPath) {
  ...
  if (!content.includes('<key>UIBackgroundModes</key>')) {
    ...
    content = content.replace('</dict>', `${insertion}</dict>`);
    changed = true;
  } else if (!content.includes('<string>remote-notification</string>')) {
    content = content.replace(
      /(<key>UIBackgroundModes<\/key>\s*<array>)([\s\S]*?)(<\/array>)/,
      (_match, open, inner, close) =>
        `${open}${inner}\t\t<string>remote-notification</string>\n\t${close}`
    );
    changed = true;
  }
