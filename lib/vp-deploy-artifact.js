...
/**
 * Deployment root. Prefer process.cwd() — on Vercel that is /var/task.
 * Do NOT use __dirname: when this module is bundled into a route chunk,
 * __dirname points at .next/server/chunks and apps/vagus-planner is missing.
 */
...
function hashLockfile(root) {
  const candidates = [
    lockfilePath(root),
    path.join(process.cwd(), 'apps', 'vagus-planner', 'package-lock.json'),
    // Legacy fallback if an older layout ever used __dirname-relative paths
    path.join(__dirname, '..', 'apps', 'vagus-planner', 'package-lock.json'),
  ];
  const tried = [];
  ...
  throw new Error(
    `ENOENT: cannot read apps/vagus-planner/package-lock.json (cwd=${process.cwd()}, __dirname=${__dirname}). Tried: ${tried.join(' | ')}`
  );
}
...
async function buildAndUploadVpDeployArtifact(options = {}) {
  ...
  try {
    ...
    log('[vp-artifact] npm ci --omit=dev … (HOME=/tmp)');
    ...
    log(`[vp-artifact] creating ${archivePath} (node-tar, no shell tar)`);
    await tar.c(
      {
        gzip: true,
        file: archivePath,
    ...
    const { error: manifestUploadError } = await supabase.storage
      ...
      .upload(manifestPath, JSON.stringify(manifest, null, 2), {
        upsert: true,
        // Bucket may not allow application/json — octet-stream is always accepted.
...
function artifactFromStorageObject(file) {
  const name = file.name || '';
  const hashMatch = name.match(/^vp-node-modules-([a-f0-9]+)\.(tar\.gz|json)$/);
  ...
  return {
    lockfile_hash: hashMatch[1],
    storage_path: `vp-node-modules-${hashMatch[1]}.tar.gz`,
    size_bytes: size,
    created_at: file.updated_at || file.created_at || new Date().toISOString(),
  };
}

async function listVpArtifactStorageFiles() {
  const supabase = createServiceClient();
...
async function getLatestVpDeployArtifact() {
  ...
  if (error) {
    console.warn('[vp-artifact] DB latest lookup error:', error.message);
  }
  if (!error && data) return data;

  // Fallback: storage objects (tar.gz naming convention — works without DB/JSON)
  const files = await listVpArtifactStorageFiles();
  const tarballs = files
    .filter((f) => f.name.startsWith('vp-node-modules-') && f.name.endsWith('.tar.gz'))
    .map(artifactFromStorageObject)
    .filter(Boolean);

  if (!tarballs.length) return null;

  tarballs.sort((a, b) => String(b.created_at).localeCompare(String(a.created_at)));
  return tarballs[0];
}
...
async function getVpDeployArtifactByHash(lockfileHash) {
  ...
  if (error) {
    console.warn(
      `[vp-artifact] DB lookup error for hash ${lockfileHash}:`,
      error.message
    );
  }
  if (!error && data) {
    console.log(`[vp-artifact] DB hit for hash ${lockfileHash}`);
    return data;
  }

  // Fallback 1: storage-side JSON manifest
  const manifestPath = `vp-node-modules-${lockfileHash}.json`;
  ...
  if (!dlError && blob) {
    try {
      ...
      console.log(`[vp-artifact] storage JSON manifest hit for hash ${lockfileHash}`);
      return {
  ...
  // Fallback 2: tarball naming convention (always works if upload succeeded)
  const storagePath = `vp-node-modules-${lockfileHash}.tar.gz`;
  try {
    const files = await listVpArtifactStorageFiles();
    const match = files.find((f) => f.name === storagePath);
    if (match) {
      const row = artifactFromStorageObject(match);
      console.log(`[vp-artifact] storage tarball hit for hash ${lockfileHash}`);
      return row;
    }
  } catch (listError) {
    console.warn('[vp-artifact] storage list fallback failed:', listError);
  }

  console.warn(`[vp-artifact] no artifact found for hash ${lockfileHash}`);
  return null;
}
...
async function downloadAndExtractVpDeployArtifact(storagePath, appDir, options = {}) {
  ...
  // Always under /tmp (not os.tmpdir()) so fallback cleanup can find and remove it.
  const archivePath = path.join('/tmp', `vp-artifact-dl-${path.basename(appDir)}.tar.gz`);
  await fsp.writeFile(archivePath, buffer);
  log(`[vp-deploy] artifact tarball written: ${archivePath} (${buffer.length} bytes)`);

  const extractStarted = Date.now();
  try {
    // Pure JS extract — Vercel functions do not ship a shell `tar` binary.
    await tar.x({
      file: archivePath,
      cwd: appDir,
    });
    ...
    await fsp.rm(archivePath, { force: true }).catch(() => {});
    log(`[vp-deploy] artifact tarball removed after extract: ${archivePath}`);
  }
  const extractMs = Date.now() - extractStarted;
  log(`[vp-deploy] artifact extract: ${extractMs}ms (node-tar)`);
