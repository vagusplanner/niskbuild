...
export function useOfflineData(entity, filter = null) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (navigator.onLine) {
        let records;
        if (filter) {
          records = await base44.entities[entity].filter(filter);
        } else {
          records = await base44.entities[entity].list('-updated_date', 200);
        }
  ...
  const create = useCallback(async (newData) => {
    ...
    if (navigator.onLine) {
      const created = await base44.entities[entity].create(newData);
  ...
  const update = useCallback(async (id, updates) => {
    ...
    if (navigator.onLine) {
      const result = await base44.entities[entity].update(id, updates);
      if (store) {
    ...
    try {
      ...
      if (action === 'create') {
        ...
        if (id && !String(id).startsWith('offline_')) {
          await base44.entities[entity].update(id, updates);
        }
