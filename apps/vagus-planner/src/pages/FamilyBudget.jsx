...
function AddEntryModal({ month, user, familyGroupId, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    await base44.entities.FamilyBudget.create({
...
export default function FamilyBudgetPage() {
  ...
  const { data: allEntries = [], isLoading } = useQuery({
    ...
    queryFn: () => group?.id ? base44.entities.FamilyBudget.filter({ family_group_id: group.id }) : [],
    enabled: !!group?.id,
  });
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await base44.functions.invoke('analyzeFamilyBudget', {
  ...
  const handleDelete = async (id) => {
    await base44.entities.FamilyBudget.delete(id);
