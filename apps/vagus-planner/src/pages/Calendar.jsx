...
export default function CalendarPage() {
  ...
  const { data: rawEvents = [], isLoading: eventsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!user?.id) return [];
      try {
        // RLS scopes rows by user_id; list() avoids nonexistent created_by column
        return await base44.entities.Event.list('-start_date', 500);
      } catch (error) {
  ...
  const { data: tasks = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await base44.entities.Task.list('-updated_date', 50);
      } catch (error) {
