...
export default function NotionImport() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const extracted = await base44.integrations.Core.ExtractDataFromUploadedFile({ file_url, json_schema: jsonSchema });
      ...
      // Bulk create
      if (importType === 'tasks') {
        await base44.entities.Task.bulkCreate(items.map(i => ({ title: i.title || 'Imported Task', description: i.description, priority: i.priority || 'medium', status: 'todo' })));
        queryClient.invalidateQueries({ queryKey: ['tasks'] });
      } else if (importType === 'goals') {
        await base44.entities.Goal.bulkCreate(items.map(i => ({ title: i.title || 'Imported Goal', description: i.description, category: i.category || 'personal', status: 'not_started' })));
