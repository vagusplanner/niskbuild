...
// Realtime subscriptions are not wired in the Supabase compat layer yet
...
/** Safe wrapper — compat layer may not implement entity.subscribe() yet */
function safeSubscribe(entityApi, callback) {
  if (!entityApi || typeof entityApi.subscribe !== 'function') {
    return () => {};
  }
  try {
    const unsubscribe = entityApi.subscribe(callback);
    return typeof unsubscribe === 'function' ? unsubscribe : () => {};
  } catch (error) {
    console.warn('Realtime subscribe unavailable:', error);
    return () => {};
  }
}
...
export default function RealTimeNotificationProvider({ children }) {
  ...
  const { data: preferences = [] } = useQuery({
    ...
    queryFn: async () => {
      try {
        return await base44.entities.NotificationPreference.list();
      } catch {
        return [];
      }
    },
  ...
  useEffect(() => {
    if (!ENABLE_REALTIME_SUBSCRIPTIONS || !user || !shouldNotify('event_reminder')) return;

    const unsubscribe = safeSubscribe(base44.entities.Event, (event) => {
  ...
  useEffect(() => {
    if (!ENABLE_REALTIME_SUBSCRIPTIONS || !user) return;

    const unsubscribeChat = safeSubscribe(base44.entities.Chat, (event) => {
    ...
    const unsubscribeGroup = safeSubscribe(base44.entities.GroupMessage, (event) => {
      if (event.type === 'create') {
        const msg = event.data;
    ...
    const unsubscribeGroupChat = safeSubscribe(base44.entities.GroupChat, (event) => {
      if (event.type === 'create') {
  ...
  useEffect(() => {
    if (!ENABLE_REALTIME_SUBSCRIPTIONS || !user || !shouldNotify('goal_update')) return;

    const unsubscribe = safeSubscribe(base44.entities.Goal, (event) => {
  ...
  useEffect(() => {
    if (!ENABLE_REALTIME_SUBSCRIPTIONS || !user || !shouldNotify('task_assigned')) return;

    const unsubscribe = safeSubscribe(base44.entities.Task, (event) => {
      if (event.type === 'create') {
  ...
  useEffect(() => {
    if (!ENABLE_REALTIME_SUBSCRIPTIONS || !user || !shouldNotify('meeting_invitation')) return;

    const unsubscribe = safeSubscribe(base44.entities.Meeting, (event) => {
      if (event.type === 'create') {
