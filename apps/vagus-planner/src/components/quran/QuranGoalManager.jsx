...
export default function QuranGoalManager() {
  ...
  const { data: readings = [] } = useQuery({
    ...
    queryFn: () => base44.entities.QuranReading.list('-date', 200)
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.QuranGoal.update(id, data),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['quranGoals'] })
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.QuranGoal.delete(id),
