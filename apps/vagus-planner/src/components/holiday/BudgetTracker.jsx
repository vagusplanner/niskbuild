...
export default function BudgetTracker({ holiday }) {
  ...
  const createMutation = useMutation({
    mutationFn: (data) => base44.entities.HolidayExpense.create(data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.HolidayExpense.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['holiday-expenses'] });
  ...
  const autoCategorizeExpense = async () => {
    ...
    try {
      const { data } = await base44.functions.invoke('categorizeExpense', {
        title: formData.title,
        amount: parseFloat(formData.amount),
        notes: formData.notes
      });
