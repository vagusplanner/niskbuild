...
export default function InteractiveAIHealthCoach() {
  ...
  const { data: workoutPlan, isLoading: loadingWorkout, refetch: refetchWorkout } = useQuery({
    ...
    queryFn: async () => {
      const response = await base44.functions.invoke('generatePersonalizedWorkoutPlan', {});
  ...
  const { data: mealPlan, isLoading: loadingMeal, refetch: refetchMeal } = useQuery({
    queryKey: ['mealPlan'],
    queryFn: async () => {
      const response = await base44.functions.invoke('generatePersonalizedMealPlan', {});
      return response.data?.data || null;
    },
    enabled: false
  });
  ...
  const { data: wellnessAnalysis, isLoading: loadingWellness, refetch: refetchWellness } = useQuery({
    ...
    queryFn: async () => {
      const response = await base44.functions.invoke('analyzeMoodSleepForWellness', {});
      return response.data?.data || null;
    },
    enabled: false
  });
  ...
  const { data: recipes, isLoading: loadingRecipes, refetch: refetchRecipes } = useQuery({
    queryKey: ['recipes'],
    queryFn: async () => {
      const response = await base44.functions.invoke('generatePersonalizedRecipes', {});
  ...
  const { data: progressReport, isLoading: loadingProgress, refetch: refetchProgress } = useQuery({
    ...
    queryFn: async () => {
      const response = await base44.functions.invoke('generateProgressReport', { timeframe: 'week' });
      return response.data?.data || null;
    },
    enabled: false
  });
  ...
  const { data: planAdjustments, isLoading: loadingAdjustments, refetch: refetchAdjustments } = useQuery({
    ...
    queryFn: async () => {
      const response = await base44.functions.invoke('suggestPlanAdjustments', { plan_type: 'general' });
      return response.data?.data || null;
    },
    enabled: false
  });
