...
export default function ScheduleCallModal({ open, onOpenChange, chat, currentUser, onScheduled }) {
  ...
  const scheduleMutation = useMutation({
    mutationFn: async () => {
      ...
      if (chat.type === 'group') {
        await base44.entities.GroupMessage.create({ ...msgData, group_chat_id: chat.id });
      } else {
        await base44.entities.Chat.create({ ...msgData, conversation_id: chat.id, is_read: false });
      }
