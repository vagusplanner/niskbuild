...
export default function IslamicLifeScore({ compact = false }) {
  ...
  const { data: prayerLogs = [] } = useQuery({
    ...
    queryFn: () => base44.entities.PrayerLog.filter({ date: { $gte: last7[0], $lte: today } }),
    staleTime: 30000,
  });

  const { data: quranReadings = [] } = useQuery({
    queryKey: ['quranReadings7', today],
    queryFn: () => base44.entities.QuranReading.filter({ created_date: { $gte: `${last7[0]}T00:00:00` } }),
    staleTime: 30000,
  });

  const { data: fastingRecords = [] } = useQuery({
    queryKey: ['fastingRecords7', today],
    queryFn: () => base44.entities.FastingRecord.filter({ date: { $gte: last7[0], $lte: today } }),
    staleTime: 30000,
  });

  const { data: expenses = [] } = useQuery({
    queryKey: ['expenses7', today],
    queryFn: () => base44.entities.Expense.filter({ date: { $gte: last7[0], $lte: today }, type: { $in: ['sadaqa', 'zakat'] } }),
    staleTime: 30000,
  });

  const { data: dhikrLogs = [] } = useQuery({
    queryKey: ['dhikrLogs7', today],
    queryFn: () => base44.entities.PrayerLog.filter({ date: { $gte: last7[0], $lte: today }, prayer_name: { $regex: 'dhikr' } }),
    staleTime: 30000,
  });
