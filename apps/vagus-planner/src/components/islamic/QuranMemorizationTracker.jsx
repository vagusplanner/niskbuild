...
export default function QuranMemorizationTracker({ compact = false }) {
  ...
  const { data: memorizations = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.QuranMemorization.list('-created_date')
  });

  const { data: goals = [] } = useQuery({
    queryKey: ['memorization-goals'],
    queryFn: () => base44.entities.MemorizationGoal.list('-created_date')
  });
  ...
  const addGoal = useMutation({
    mutationFn: async (data) => {
      const goal = await base44.entities.MemorizationGoal.create(data);
  ...
  const handleStatusChange = async (mem, newStatus) => {
    ...
    if (newStatus === 'memorized' && !mem.memorized_date) {
      ...
      try {
        const result = await base44.functions.invoke('awardMemorizationPoints', {
    ...
    if (newStatus === 'reviewing') {
      ...
      try {
        await base44.functions.invoke('awardMemorizationPoints', {
