...
export default function PrayerReflectionJournal({ prayerLog = null, prayerName = 'Daily', date = null, trigger = null }) {
  ...
  const { data: reflections = [] } = useQuery({
    ...
    queryFn: () => base44.entities.PrayerReflection.filter({ date: historyDate }),
    enabled: open,
    initialData: []
  });
  ...
  const { data: todayReflection = null } = useQuery({
    ...
    queryFn: async () => {
      const res = await base44.entities.PrayerReflection.filter({ date: targetDate, prayer_name: prayerName });
      return res[0] || null;
    },
    enabled: open,
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (data) => base44.entities.PrayerReflection.create(data),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.PrayerReflection.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['prayer-reflections'] });
      queryClient.invalidateQueries({ queryKey: ['prayer-reflection-check'] });
