...
export default function GroupPilgrimageManager() {
  ...
  const { data: groups = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const allGroups = await base44.entities.PilgrimageGroup.list('-created_date');
  ...
  const createGroupMutation = useMutation({
    mutationFn: async (data) => {
      const user = await base44.auth.me();
      return base44.entities.PilgrimageGroup.create({
        ...data,
  ...
  const deleteGroupMutation = useMutation({
    mutationFn: (id) => base44.entities.PilgrimageGroup.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['pilgrimageGroups'] });
