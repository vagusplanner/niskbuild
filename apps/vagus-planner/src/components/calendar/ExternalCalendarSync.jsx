...
import { RefreshCw, Calendar, CheckCircle, AlertCircle, Settings, Download, Upload, Zap } from 'lucide-react';
...
export default function ExternalCalendarSync({ onSyncComplete }) {
  ...
  const handleSync = async (type, action = 'import') => {
    setSyncing(true);
    try {
      if (type === 'google') {
        if (action === 'export' || action === 'two_way_sync') {
          toast.info('Export to Google Calendar is not available yet (one-way pull only).');
          setSyncing(false);
          return;
        }
        ...
        setSyncStats({
          imported: response.data.imported || response.data.syncedCount || 0,
          exported: 0
        });
        setLastSync(new Date());
        toast.success(`Imported ${response.data.imported || response.data.syncedCount || 0} events from Google Calendar`);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
            ...
            <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 rounded-lg border border-blue-200 dark:border-blue-800">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400">Pull when you visit Calendar</p>
                </div>
              </div>
              <Switch
                id="auto-sync"
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">One-way pull from primary calendar</p>
                </div>
              </div>
            </div>
        ...
        <div className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 p-3 rounded-lg border border-slate-200 dark:border-slate-700">
          <p className="mb-2"><strong>One-way pull:</strong> Imports Google events into Vagus Planner</p>
          <ul className="list-disc list-inside space-y-1 text-slate-600 dark:text-slate-400">
            <li>Connect Google Calendar from Account → Calendar Integrations</li>
            <li>Export / write-back to Google is not available yet</li>
