...
export default function AICollaborationTools({ holiday }) {
  ...
  const generateSummary = async () => {
    ...
    try {
      const { data } = await base44.functions.invoke('generateCollaborationSummary', {
  ...
  const suggestMeetingTimes = async () => {
    ...
    try {
      const { data } = await base44.functions.invoke('suggestMeetingTimes', {
        holiday_id: holiday.id,
        duration_minutes: 60
      });
  ...
  const draftProposal = async () => {
    ...
    try {
      const { data } = await base44.functions.invoke('draftExpenseProposal', {
        holiday_id: holiday.id,
        ...proposalData
      });
