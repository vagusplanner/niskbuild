...
export default function SocialScheduler() {
  ...
  const { data: connections = [] } = useQuery({
    ...
    queryFn: () => base44.entities.SocialConnection.filter({ status: 'active' })
  });

  const { data: suggestions = [] } = useQuery({
    queryKey: ['social-suggestions'],
    queryFn: () => base44.entities.SocialSuggestion.filter({ status: 'active' }, '-created_date', 10)
  });

  const processCommandMutation = useMutation({
    mutationFn: async (cmd) => {
      return base44.functions.invoke('processSocialCommand', { command: cmd });
    },
    onSuccess: (response) => {
  ...
  const analyzeAvailabilityMutation = useMutation({
    mutationFn: async (friendEmail) => {
      return base44.functions.invoke('analyzeSocialAvailability', { 
  ...
  const generateStartersMutation = useMutation({
    mutationFn: async (friendEmail) => {
      return base44.functions.invoke('generateConversationStarters', { 
        friend_email: friendEmail 
      });
    },
    onSuccess: () => {
