...
const Select = ({ children, value, onValueChange, ...props }) => {
  ...
  React.useEffect(() => {
    if (isMobile) {
      ...
      setItems(extractItems(children));
    }
  }, [children, isMobile]);
  ...
  if (isMobile) {
    ...
    return (
      <SelectContext.Provider value={{ items, open, setOpen }}>
              ...
              <div className="px-4 pb-6 max-h-[60vh] overflow-auto">
                {items.map((item) => (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between px-4 py-3 text-left rounded-lg mb-2 transition-colors",
                      value === item.value 
                  ...
                  >
                    ...
                    {value === item.value && <Check className="w-5 h-5" />}
                  </button>
                ))}
              </div>
            </DrawerContent>
...
const SelectContent = React.forwardRef(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ...
      position={position}
      style={{ zIndex: SELECT_MENU_Z_INDEX }}
      {...props}>
