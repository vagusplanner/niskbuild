...
function ClaimDialog({ mosque, user, onClose, onClaimed }) {
  ...
  const claimMutation = useMutation({
    mutationFn: async () => {
      ...
      const existing = await base44.entities.MosaqueDirectory.filter({ osm_id: String(mosque.id) });
      const data = {
      ...
      if (existing.length > 0) {
        return base44.entities.MosaqueDirectory.update(existing[0].id, { ...data, claim_pending: true });
      }
      return base44.entities.MosaqueDirectory.create(data);
    },
    onSuccess: () => {
...
function MosqueMapContent() {
  ...
  const { data: directoryEntries = [] } = useQuery({
    ...
    queryFn: () => base44.entities.MosaqueDirectory.list('-created_date', 200)
  });
