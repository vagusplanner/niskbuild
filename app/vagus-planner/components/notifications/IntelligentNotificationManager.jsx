...
export default function IntelligentNotificationManager() {
  ...
  const { data: notifications = [] } = useQuery({
    ...
    queryFn: () => base44.entities.SmartNotification.list('-scheduled_time', 50)
  });
  ...
  const updatePreferenceMutation = useMutation({
    mutationFn: ({ id, data }) => 
      id ? base44.entities.NotificationPreference.update(id, data) 
         : base44.entities.NotificationPreference.create(data),
    onSuccess: () => {
  ...
  const markAsReadMutation = useMutation({
    mutationFn: (id) => base44.entities.SmartNotification.update(id, { status: 'read' }),
    onSuccess: () => {
  ...
  const dismissMutation = useMutation({
    mutationFn: (id) => base44.entities.SmartNotification.update(id, { status: 'dismissed' }),
    onSuccess: () => {
      queryClient.invalidateQueries(['smartNotifications']);
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => base44.entities.SmartNotification.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries(['smartNotifications']);
