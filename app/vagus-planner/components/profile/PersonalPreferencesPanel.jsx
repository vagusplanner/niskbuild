...
export default function PersonalPreferencesPanel() {
  ...
  const handleIslamicModeChange = (value) => {
    const updated = { ...form, islamic_mode: value };
    setForm(updated);
    // Persist immediately to localStorage so nav shows correct state before DB round-trip
    try { localStorage.setItem('vagus_islamic_mode', value ? '1' : '0'); } catch (e) {}
    // Auto-save to DB
    const save = settings?.id
      ? base44.entities.UserSettings.update(settings.id, updated)
      : base44.entities.UserSettings.create(updated);
    save.then(() => {
      queryClient.invalidateQueries({ queryKey: ['userSettings'] });
