...
export default function SubscriptionNotificationSettings() {
  ...
  const { data: settings = [] } = useQuery({
    ...
    queryFn: () => base44.asServiceRole.entities.SubscriptionNotificationSettings.list()
  });

  const { data: emailLogs = [] } = useQuery({
    queryKey: ['subscriptionEmailLogs'],
    queryFn: () => base44.asServiceRole.entities.SubscriptionEmailLog.list('-sent_date', 50)
  });
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async (data) => {
      if (settings.length > 0) {
        return base44.asServiceRole.entities.SubscriptionNotificationSettings.update(
          settings[0].id,
          {
            ...data,
        ...
        return base44.asServiceRole.entities.SubscriptionNotificationSettings.create({
          ...data,
          last_updated: new Date().toISOString()
        });
      }
