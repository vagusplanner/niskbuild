...
Journal entry: "${transcript}"`,
  ...
  const syncAll = async () => {
    ...
    try {
      ...
      const habitMatches = await base44.entities.Habit.filter({ category: 'spiritual' }, '-created_date', 50);
      ...
      if (journalHabit) {
        ...
        if (!completions.includes(today)) {
          await base44.entities.Habit.update(journalHabit.id, { completion_dates: [...completions, today] });
        }
      }
