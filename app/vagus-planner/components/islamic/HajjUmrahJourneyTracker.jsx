...
export default function HajjUmrahJourneyTracker() {
  ...
  const { data: rituals = [] } = useQuery({
    ...
    queryFn: () => base44.entities.HajjRitual.filter({ type: activeJourney })
  });

  const { data: group } = useQuery({
    queryKey: ['pilgrimageGroup'],
    queryFn: async () => {
      const groups = await base44.entities.PilgrimageGroup.list();
      return groups[0];
    }
  });

  const completeRitualMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.HajjRitual.update(id, data),
    onSuccess: () => {
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (existing) {
                              ...
                              base44.entities.HajjRitual.create({
