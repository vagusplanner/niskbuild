...
export default function PrayerTimeSettings() {
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async (updates) => {
      if (settings.length > 0) {
        return base44.entities.UserSettings.update(settings[0].id, updates);
      } else {
        return base44.entities.UserSettings.create(updates);
      }
    },
    onSuccess: () => {
  ...
  const detectLocationMutation = useMutation({
    mutationFn: async () => {
      const response = await base44.functions.invoke('detectUserLocation', {});
