...
export default function GroupTaskBoard({ group, members = [] }) {
  ...
  const { data: tasks = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const allTasks = await base44.entities.HajjGroupTask.list();
      return allTasks.filter(t => t.group_id === group.id);
    }
  });

  const createTaskMutation = useMutation({
    mutationFn: (data) => base44.entities.HajjGroupTask.create({
      ...data,
      group_id: group.id,
  ...
  const updateTaskMutation = useMutation({
    mutationFn: ({ id, data }) => base44.entities.HajjGroupTask.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['groupTasks', group.id] });
    }
  });
  ...
  const deleteTaskMutation = useMutation({
    mutationFn: (id) => base44.entities.HajjGroupTask.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['groupTasks', group.id] });
