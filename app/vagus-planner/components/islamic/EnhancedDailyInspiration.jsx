...
export default function EnhancedDailyInspiration({ compact = false }) {
  ...
  const loadVerse = async (today) => {
    const existingVerses = await base44.entities.QuranVerse.filter({ date: today });
    ...
    const events = await base44.entities.Event.list('-start_date', 10);
    
    const result = await base44.functions.invoke('generateDailyQuranVerse', {
      date: today,
  ...
  const loadReflection = async () => {
    const settings = await base44.entities.UserSettings.list();
    const events = await base44.entities.Event.list('-start_date', 10);
    const tasks = await base44.entities.Task.list('-updated_date', 10);
