...
export default function QuranReader({ initialSurah = 1, initialVerse = 1, onClose = () => {} }) {
  ...
  const { data: todayReadings = [] } = useQuery({
    ...
    queryFn: () => base44.entities.QuranReading.filter({ 
  ...
  const logReadingMutation = useMutation({
    mutationFn: async (data) => {
      const reading = await base44.entities.QuranReading.create(data);
      ...
      if (activeGoal) {
        ...
        await base44.entities.QuranGoal.update(activeGoal.id, {
