...
function ManualSnapshotForm({ onSave, onClose }) {
  ...
  const saveMutation = useMutation({
    mutationFn: () => base44.entities.ZakatTracking.create({
...
export default function AutoZakatTracker() {
  ...
  const { data: snapshots = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const all = await base44.entities.ZakatTracking.list('-snapshot_date', 30);
      return all;
    }
  });
  ...
  const handleAutoSnapshot = async () => {
    ...
    try {
      const res = await base44.functions.invoke('autoZakatAnalysis', { action: 'auto_snapshot', currency: CURRENCY });
      queryClient.invalidateQueries({ queryKey: ['zakatSnapshots'] });
  ...
  const handleGetSuggestions = async () => {
    ...
    try {
      const res = await base44.functions.invoke('autoZakatAnalysis', { action: 'get_suggestions', currency: CURRENCY });
  ...
  const markZakatPaid = useMutation({
    mutationFn: (snapshotId) => base44.entities.ZakatTracking.update(snapshotId, {
      zakat_paid: true,
