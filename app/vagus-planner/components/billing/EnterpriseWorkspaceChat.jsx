...
export default function EnterpriseWorkspaceChat() {
  ...
  const { data: groups = [] } = useQuery({
    ...
    queryFn: () => base44.entities.GroupChat.filter({ context_type: 'general' }),
  ...
  const { data: messages = [] } = useQuery({
    ...
    queryFn: () => selectedChat
      ? base44.entities.GroupMessage.filter({ group_chat_id: selectedChat.id }, '-created_date', 50)
      : [],
  ...
  const createGroupMutation = useMutation({
    mutationFn: () => base44.entities.GroupChat.create({
      name: newGroupName,
  ...
  const sendMessageMutation = useMutation({
    mutationFn: () => base44.entities.GroupMessage.create({
