...
export default function FamilyGroceryList({ groupId, user }) {
  ...
  const { data: items = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.GroceryItem.filter({ family_group_id: groupId }),
    enabled: !!groupId,
  });

  const addItem = useMutation({
    mutationFn: (data) => base44.entities.GroceryItem.create(data),
    onSuccess: () => qc.invalidateQueries(['groceryItems', groupId]),
  });

  const toggleItem = useMutation({
    mutationFn: ({ id, is_checked }) => base44.entities.GroceryItem.update(id, { is_checked, checked_by: user?.email }),
    onSuccess: () => qc.invalidateQueries(['groceryItems', groupId]),
  });
  ...
  const clearChecked = async () => {
    ...
    await Promise.all(checked.map(i => base44.entities.GroceryItem.delete(i.id)));
