...
export default function FamilyCalendarApproval({ groupId, user, isAdmin, memberEmails = [], memberNames = [] }) {
  ...
  const { data: events = [], isLoading } = useQuery({
    ...
    queryFn: () => base44.entities.FamilyCalendarEvent.filter({ family_group_id: groupId }, '-event_date', 50),
    enabled: !!groupId,
  });

  const createEvent = useMutation({
    mutationFn: (data) => base44.entities.FamilyCalendarEvent.create(data),
    onSuccess: (created) => {
  ...
  const respondEvent = useMutation({
    mutationFn: ({ id, decision, event }) => {
      ...
      return base44.entities.FamilyCalendarEvent.update(id, { approvals: newApprovals, approval_status: newStatus });
    },
