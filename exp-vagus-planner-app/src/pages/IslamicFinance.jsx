...
function ZakatTab() {
  ...
  const { data: history = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.ZakatCalculation.list('-created_date', 10),
  });
  const { data: donations = [] } = useQuery({
    queryKey: ['charityDonations'],
    queryFn: () => SDK.entities.CharityDonation.list('-date', 50),
  });

  const saveMutation = useMutation({
    mutationFn: (data) => SDK.entities.ZakatCalculation.create(data),
  ...
  const calculate = async () => {
    setCalculating(true);
    try {
      const nisabInfo = await SDK.integrations.Core.InvokeLLM({
...
function CharityTab() {
  ...
  const saveMutation = useMutation({
    mutationFn: (d) => SDK.entities.CharityDonation.create(d),
    onSuccess: () => {
