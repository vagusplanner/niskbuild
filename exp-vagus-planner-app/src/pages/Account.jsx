...
export default function Account() {
  ...
  const { data: usageData = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await SDK.entities.Usage.filter({ user_email: user.email });
    },
    enabled: !!user?.email
  });
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async (data) => {
      if (settings?.id) {
        return SDK.entities.UserSettings.update(settings.id, data);
      } else {
            ...
            {activeSection === 'billing' && (
              <div className="space-y-4">
                <EnhancedSubscriptionCard subscription={subscription||{}} usageData={usageData}
                  onManage={async()=>{try{const{data}=await SDK.functions.invoke('createCustomerPortalSession');if(data?.portalUrl)window.location.href=data.portalUrl;else toast.error(data?.error||'Failed')}catch{toast.error('Failed')}}}
                  onUpgrade={()=>window.location.href='/Billing'}
                  onCancel={async()=>{if(subscription?.stripe_subscription_id){await SDK.functions.invoke('cancelStripeSubscription',{subscriptionId:subscription.stripe_subscription_id,reason:'User requested'});queryClient.invalidateQueries({queryKey:['subscription']});toast.success('Cancelled');}}} />
