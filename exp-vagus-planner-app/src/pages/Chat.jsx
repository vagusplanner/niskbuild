...
export default function ChatPage() {
  ...
  const { data: directMessages = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const messages = await SDK.entities.Chat.list('-created_date', 500);
  ...
  const { data: groupMessages = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.GroupMessage.list('-created_date', 500),
    refetchInterval: 3000
  });
  ...
  const { data: groupChats = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!currentUser) return [];
      const chats = await SDK.entities.GroupChat.list('-created_date');
  ...
  const { data: contacts = [] } = useQuery({
    queryKey: ['contacts'],
    queryFn: () => SDK.entities.SocialConnection.filter({ 
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async ({ message, attachments }) => {
      ...
      if (selectedChat.type === 'direct') {
        ...
        if (contact) {
          SDK.entities.SocialConnection.update(contact.id, {
            last_interacted: new Date().toISOString()
          }).catch(console.error);
        }
        
        return SDK.entities.Chat.create({
          ...messageData,
  ...
  const handleSyncGmail = async () => {
    ...
    try {
      const result = await SDK.functions.invoke('syncGmailContacts', {});
      queryClient.invalidateQueries({ queryKey: ['contacts'] });
  ...
  const handleSyncPhone = async (contactsData) => {
    ...
    try {
      const result = await SDK.functions.invoke('syncPhoneContacts', { contacts: contactsData });
      queryClient.invalidateQueries({ queryKey: ['contacts'] });
