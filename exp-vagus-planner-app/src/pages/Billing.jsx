...
export default function BillingPage() {
  ...
  const upgradeMutation = useMutation({
    mutationFn: async ({ planId, planName, billingCycle, overridePriceId }) => {
      ...
      try {
        const { data } = await SDK.functions.invoke('createStripeCheckout', {
          priceId,
          planName,
          billingCycle
        });
  ...
  const cancelMutation = useMutation({
    mutationFn: async () => {
      if (subscription?.stripe_subscription_id) {
        await SDK.functions.invoke('cancelStripeSubscription', {
  ...
  const handleManageSubscription = async () => {
    try {
      ...
      const { data } = await SDK.functions.invoke('createCustomerPortalSession');
