...
function ChallengeCard({ card, onRate, onSkip, current, total }) {
  ...
  const fetchAI = async () => {
    setLoadingAI(true);
    try {
      const res = await SDK.functions.invoke('hadithSRSChallenge', {
        action: 'ai_challenge',
...
export default function HadithLearningPage() {
  ...
  const { data: dailyData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await SDK.functions.invoke('hadithSRSChallenge', { action: 'get_daily' });
      return res.data;
    }
  });
  ...
  const { data: challenges = [] } = useQuery({
    ...
    queryFn: () => user?.email
      ? SDK.entities.HadithChallenge.filter({ created_by: user.email }, '-challenge_date', 30)
      : [],
    enabled: !!user?.email,
  });
  ...
  const handleRate = async (card, quality) => {
    try {
      await SDK.functions.invoke('hadithSRSChallenge', { action: 'review', card_id: card.id, quality });
  ...
  const finishSession = async () => {
    ...
    try {
      if (todayChallenge) {
        await SDK.entities.HadithChallenge.update(todayChallenge.id, {
          score: avgScore, correct, total, completed: true,
          hadith_ids_reviewed: reviewQueue.map(c => c.id),
        });
      } else {
        await SDK.entities.HadithChallenge.create({
