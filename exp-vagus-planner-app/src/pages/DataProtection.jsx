...
export default function DataProtectionPage() {
  ...
  const exportMyData = async () => {
    ...
    try {
      ...
      const [events, tasks, prayerLogs, routines, settings, goals] = await Promise.all([
        ...
        SDK.entities.PrayerLog.list(),
        SDK.entities.DailyRoutine.list(),
        SDK.entities.UserSettings.list(),
        SDK.entities.Goal.list()
      ]);
  ...
  const deleteMyAccount = async () => {
    ...
    try {
      ...
      // Delete all records
      await Promise.all([
        ...events.map(e => SDK.entities.Event.delete(e.id)),
        ...tasks.map(t => SDK.entities.Task.delete(t.id)),
        ...prayerLogs.map(p => SDK.entities.PrayerLog.delete(p.id)),
        ...routines.map(r => SDK.entities.DailyRoutine.delete(r.id)),
        ...settings.map(s => SDK.entities.UserSettings.delete(s.id))
      ]);
