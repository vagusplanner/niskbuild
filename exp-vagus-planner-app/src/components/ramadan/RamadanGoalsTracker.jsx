...
export default function RamadanGoalsTracker() {
  ...
  const { data: goals = [], isLoading } = useQuery({
    ...
    queryFn: () => SDK.entities.RamadanGoal.filter({ status: 'active' }),
  });
  ...
  const createGoal = async () => {
    ...
    setSaving(true);
    await SDK.entities.RamadanGoal.create({
      ...form,
      current_value: 0,
      status: 'active',
      ramadan_year: 1447,
  ...
  const updateGoal = async (goal, newValue) => {
    ...
    await SDK.entities.RamadanGoal.update(goal.id, {
  ...
  const deleteGoal = async (id) => {
    await SDK.entities.RamadanGoal.delete(id);
    await queryClient.invalidateQueries({ queryKey: ['ramadanGoals'] });
  };
