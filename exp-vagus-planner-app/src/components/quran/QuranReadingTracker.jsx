...
export default function QuranReadingTracker() {
  ...
  const { data: readings = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.QuranReading.list('-date', 500)
  });
  ...
  const goalMutation = useMutation({
    mutationFn: async (data) => {
      if (activeGoal) {
        return SDK.entities.QuranGoal.update(activeGoal.id, data);
      }
      return SDK.entities.QuranGoal.create({ ...data, status: 'active', streak: 0, best_streak: 0, total_verses_read: 0, title: 'My Quran Reading Goal' });
    },
    onSuccess: () => {
