...
export default function SmartNotificationCenter() {
  ...
  const { data: notifications = [] } = useQuery({
    ...
    queryFn: () => user
      ? SDK.entities.Notification.filter({ recipient_email: user.email }, '-created_date', 50)
      : [],
    enabled: !!user,
  ...
  const markAllRead = useMutation({
    mutationFn: async () => {
      ...
      await Promise.all(unread.map(n => SDK.entities.Notification.update(n.id, { is_read: true })));
    },
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['notifications'] })
  });

  const markRead = useMutation({
    mutationFn: (id) => SDK.entities.Notification.update(id, { is_read: true }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['notifications'] })
  });

  const deleteNotif = useMutation({
    mutationFn: (id) => SDK.entities.Notification.delete(id),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['notifications'] })
  });
  ...
  const togglePref = useMutation({
    mutationFn: async ({ type, enabled }) => {
      ...
      if (existing) {
        return SDK.entities.NotificationPreference.update(existing.id, { enabled });
      }
      return SDK.entities.NotificationPreference.create({
  ...
  const handleSnooze = async (notif, minutes) => {
    await SDK.entities.Notification.update(notif.id, { is_read: true });
