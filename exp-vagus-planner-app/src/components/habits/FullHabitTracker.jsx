...
export default function FullHabitTracker() {
  ...
  const { data: habits = [], isLoading } = useQuery({
    queryKey: ['habits'],
    queryFn: () => SDK.entities.Habit.list('-created_date')
  });

  const { data: completions = [] } = useQuery({
    queryKey: ['habitCompletions'],
    queryFn: () => SDK.entities.HabitCompletion.list('-completion_date', 500)
  });
  ...
  const toggleMutation = useMutation({
    mutationFn: ({ habit, isCompleting }) => {
      ...
      if (isCompleting && !existing) {
        return SDK.entities.HabitCompletion.create({ habit_id: habit.id, completion_date: dateStr, count: 1 });
      } else if (!isCompleting && existing) {
        return SDK.entities.HabitCompletion.delete(existing.id);
      }
    },
  ...
  const skipMutation = useMutation({
    mutationFn: ({ habit, reason }) => {
      const dateStr = format(new Date(), 'yyyy-MM-dd');
      return SDK.entities.HabitCompletion.create({
        habit_id: habit.id,
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => SDK.entities.Habit.delete(id),
    onSuccess: () => {
  ...
  const quickAddMutation = useMutation({
    mutationFn: (habit) => SDK.entities.Habit.create({ ...habit, is_active: true, target_count: 1 }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['habits'] });
