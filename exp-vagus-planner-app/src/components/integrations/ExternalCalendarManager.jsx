...
export default function ExternalCalendarManager() {
  ...
  const syncGoogleMutation = useMutation({
    mutationFn: async (calendarId = 'primary') => {
      const response = await SDK.functions.invoke('syncGoogleCalendar', {
        calendarId
      });
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: async (updates) => {
      ...
      await SDK.entities.UserSettings.update(userSettings.id, updates);
    },
    onSuccess: () => {
