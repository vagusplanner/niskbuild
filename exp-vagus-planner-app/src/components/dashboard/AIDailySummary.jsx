...
export default function AIDailySummary() {
  ...
  const { data: todayEvents = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return SDK.entities.Event.filter({
  ...
  const { data: priorityTasks = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.Task.filter({ 
  ...
  const { data: recentSleep = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.Sleep.list('-date', 7)
  });

  const { data: recentMood = [] } = useQuery({
    queryKey: ['recent-mood'],
    queryFn: () => SDK.entities.Mood.list('-date', 7)
  });

  const { data: activeTrips = [] } = useQuery({
    queryKey: ['active-trips-summary'],
    queryFn: async () => {
      const all = await SDK.entities.Holiday.list();
