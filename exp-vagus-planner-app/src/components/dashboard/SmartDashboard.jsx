...
export default function SmartDashboard() {
  ...
  const { data: briefing } = useQuery({
    ...
    queryFn: async () => {
      const briefings = await SDK.entities.DailyBriefing.filter({ date: today }, '-created_date', 1);
      return briefings[0];
    }
  });
  ...
  const { data: energyLogs = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.EnergyLog.list('-date', 7)
  });

  const { data: habits = [] } = useQuery({
    queryKey: ['active-habits'],
    queryFn: () => SDK.entities.Habit.filter({ is_active: true })
  });

  const { data: financialEvents = [] } = useQuery({
    queryKey: ['upcoming-bills'],
    queryFn: () => SDK.entities.FinancialEvent.filter({ 
      status: 'pending',
      due_date: { $gte: today }
    }, 'due_date', 5)
  });
