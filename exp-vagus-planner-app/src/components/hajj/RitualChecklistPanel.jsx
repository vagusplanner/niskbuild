...
export default function RitualChecklistPanel({ tripId, tripType = 'hajj' }) {
  ...
  const { data: completedRituals = [] } = useQuery({
    ...
    queryFn: async () => {
      const progress = await SDK.entities.HajjRitual.filter({ trip_id: tripId });
      return progress;
    },
    enabled: !!tripId
  });
  ...
  const toggleRitual = useMutation({
    mutationFn: async ({ ritualId, completed }) => {
      ...
      if (existing) {
        if (!completed) {
          await SDK.entities.HajjRitual.delete(existing.id);
        }
      } else {
        await SDK.entities.HajjRitual.create({
          trip_id: tripId,
          ritual_id: ritualId,
