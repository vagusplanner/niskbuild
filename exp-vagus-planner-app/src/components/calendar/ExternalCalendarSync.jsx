...
export default function ExternalCalendarSync({ onSyncComplete }) {
  ...
  const updateSettingsMutation = useMutation({
    mutationFn: (data) => {
      ...
      if (settingsId) {
        return SDK.entities.UserSettings.update(settingsId, data);
      }
  ...
  const handleSync = async (type, action = 'two_way_sync') => {
    ...
    try {
      if (type === 'google') {
        const response = await SDK.functions.invoke('enhancedCalendarSync', {
          action,
  ...
  const detectConflicts = async () => {
    try {
      ...
      for (const event of events.slice(0, 10)) {
        const response = await SDK.functions.invoke('detectCrossCalendarConflicts', {
          event_id: event.id
        });
