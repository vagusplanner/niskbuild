...
export default function TravelAwareAlert() {
  ...
  const { data: holidays = [] } = useQuery({
    queryKey: ['holidays'],
    queryFn: () => SDK.entities.Holiday.filter({ status: { $in: ['booked', 'in_progress', 'planned'] } }),
    staleTime: 60000,
  });
  const { data: events = [] } = useQuery({
    ...
    queryFn: () => {
      ...
      return SDK.entities.Event.filter({ start_date: { $gte: `${today}T00:00:00Z`, $lte: `${next14}T23:59:59Z` } });
    },
    staleTime: 30000,
  });
