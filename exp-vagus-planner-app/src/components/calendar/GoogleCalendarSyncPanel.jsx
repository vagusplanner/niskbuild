...
export default function GoogleCalendarSyncPanel() {
  ...
  const { data: syncStates = [] } = useQuery({
    queryKey: ['syncState'],
    queryFn: () => SDK.entities.SyncState.filter({ service: 'googlecalendar' }),
    refetchInterval: 30000,
  });

  const { data: gcalEvents = [] } = useQuery({
    queryKey: ['gcal-events-count'],
    queryFn: () => SDK.entities.Event.filter({ source: 'google_calendar' }, '-start_date', 5),
    staleTime: 30000,
  });
  ...
  const handleFullSync = async () => {
    setSyncing(true);
    try {
      const res = await SDK.functions.invoke('initialGCalSync', {});
