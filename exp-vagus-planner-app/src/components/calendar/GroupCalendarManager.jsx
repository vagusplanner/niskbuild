...
export default function GroupCalendarManager({ isOpen, onClose }) {
  ...
  const { data: myGroups = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.GroupCalendar.filter({ 
  ...
  const { data: sharedGroups = [] } = useQuery({
    ...
    queryFn: async () => {
      const shares = await SDK.entities.SharedCalendar.filter({ 
      ...
      const groups = await Promise.all(
        groupIds.map(id => SDK.entities.GroupCalendar.filter({ id }))
      );
      return groups.flat();
    },
    enabled: !!user
  ...
  const createGroupMutation = useMutation({
    mutationFn: (data) => SDK.entities.GroupCalendar.create({
      ...data,
      owner_email: user.email
    }),
    onSuccess: () => {
  ...
  const deleteGroupMutation = useMutation({
    mutationFn: (id) => SDK.entities.GroupCalendar.update(id, { is_active: false }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['my-group-calendars'] });
