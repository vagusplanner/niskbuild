...
export default function AISchedulePlanner({ isOpen, onClose, onEventsCreated }) {
  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: () => user?.email
      ? SDK.entities.Event.filter({ created_by: user.email }, '-start_date', 300)
      : Promise.resolve([]),
    enabled: !!user?.email,
  });
  const { data: tasks = [] } = useQuery({
    queryKey: ['tasks'],
    queryFn: () => SDK.entities.Task.list('-updated_date', 50),
  });
  ...
  const generate = async (overrideStyle = null) => {
    ...
    try {
      const res = await SDK.functions.invoke('aiSchedulePlanner', {
        period,
        style,
  ...
  const applySchedule = async (eventsToCreate) => {
    ...
    for (const ev of eventsToCreate) {
      try {
        await SDK.entities.Event.create(ev);
        created++;
