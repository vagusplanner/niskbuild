...
export default function ConflictResolutionModal({ conflict, onClose, onResolve, events = [] }) {
  ...
  const generateAISuggestions = async () => {
    ...
    try {
      ...
      const { data } = await SDK.functions.invoke('detectConflicts', {
  ...
  const applyMutation = useMutation({
    mutationFn: async ({ suggestion }) => {
      if (suggestion.type === 'delegate') {
        ...
        await SDK.entities.Event.update(suggestion.event_id, {
          start_date: suggestion.new_start_date,
          end_date: suggestion.new_end_date,
        });
      }
      if (conflict?.id) {
        await SDK.entities.ConflictResolution.update(conflict.id, {
