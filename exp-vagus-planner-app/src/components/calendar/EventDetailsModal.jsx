...
export default function EventDetailsModal({ event, isOpen, onClose, onEdit, onDelete, onCreateTask }) {
  ...
  const fetchRelatedTasks = async () => {
    try {
      const tasks = await SDK.entities.Task.filter({ event_id: event.id });
  ...
  const handlePushToGoogle = async () => {
    ...
    try {
      ...
      const res = await SDK.functions.invoke('pushEventToGoogleCalendar', {
        event_id: event.id,
        action,
      });
  ...
  const handleSendEmail = async () => {
    ...
    try {
      await SDK.functions.invoke('sendCalendarInvite', {
        event_id: event.id,
                    ...
                    {relatedTasks.map(task => (
                      <div key={task.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        <input
                          ...
                          onChange={async () => {
                            await SDK.entities.Task.update(task.id, {
