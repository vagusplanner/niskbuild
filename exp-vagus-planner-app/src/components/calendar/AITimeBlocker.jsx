...
export default function AITimeBlocker({ onEventCreated, islamicMode = false }) {
  ...
  const { data: tasks = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.Task.filter({ status: { $in: ['todo', 'in_progress'] } }, '-priority', 20),
  });
  const { data: events = [] } = useQuery({
    ...
    queryFn: () => {
      ...
      return SDK.entities.Event.filter({ start_date: { $gte: `${today}T00:00:00Z`, $lte: `${next7}T23:59:59Z` } });
    },
  });
