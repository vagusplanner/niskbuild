...
export default function TeamManager({ isOpen, onClose }) {
  ...
  const { data: teams = [] } = useQuery({
    queryKey: ['teams'],
    queryFn: () => SDK.entities.Team.list('-created_date')
  });

  const { data: allMembers = [] } = useQuery({
    queryKey: ['teamMembers'],
    queryFn: () => SDK.entities.TeamMember.list()
  });

  const createTeamMutation = useMutation({
    mutationFn: async (data) => {
      const team = await SDK.entities.Team.create(data);
  ...
  const inviteMemberMutation = useMutation({
    mutationFn: (data) => SDK.entities.TeamMember.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['teamMembers'] });
      setInviteEmail('');
  ...
  const removeMemberMutation = useMutation({
    mutationFn: (id) => SDK.entities.TeamMember.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['teamMembers'] });
