...
export default function EventCollaborationPanel({ eventId, onClose }) {
  ...
  const { data: comments = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.EventComment.filter({ event_id: eventId }),
    enabled: !!eventId
  });
  ...
  const { data: edits = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.EventEdit.filter({ event_id: eventId }),
    enabled: !!eventId,
  ...
  useEffect(() => {
    ...
    const unsubComments = SDK.entities.EventComment.subscribe((event) => {
      if (event.data?.event_id === eventId) {
        queryClient.invalidateQueries({ queryKey: ['eventComments', eventId] });
      }
    });

    const unsubEdits = SDK.entities.EventEdit.subscribe((event) => {
      if (event.data?.event_id === eventId) {
  ...
  const sendCommentMutation = useMutation({
    mutationFn: (data) => SDK.entities.EventComment.create(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['eventComments', eventId] });
      setMessage('');
    }
