...
export default function RealTimeGroupChat({ groupChatId, contextType, contextId }) {
  ...
  useEffect(() => {
    SDK.auth.me().then(setUser);
  }, []);

  // Real-time subscription to messages
  const { data: messages = [], isLoading } = useQuery({
    queryKey: ['groupMessages', groupChatId],
    queryFn: () => SDK.entities.GroupMessage.filter({ group_chat_id: groupChatId }, '-created_date', 100),
    enabled: !!groupChatId,
  ...
  const { data: groupChat } = useQuery({
    ...
    queryFn: () => SDK.entities.GroupChat.get(groupChatId),
    enabled: !!groupChatId
  });
  ...
  useEffect(() => {
    ...
    const unsubscribe = SDK.entities.GroupMessage.subscribe((event) => {
  ...
  const sendMessageMutation = useMutation({
    mutationFn: (messageData) => SDK.entities.GroupMessage.create(messageData),
    onSuccess: () => {
      setMessage('');
      queryClient.invalidateQueries({ queryKey: ['groupMessages', groupChatId] });
    },
