...
async function saveExtracted(extracted, transcript, queryClient) {
  ...
  if (cat === 'task' || cat === 'islamic') {
    const record = await SDK.entities.Task.create({
  ...
  if (cat === 'goal') {
    const record = await SDK.entities.Goal.create({
      title: extracted.title || transcript.substring(0, 80),
      description: extracted.description || transcript,
      priority: extracted.priority || 'medium',
  ...
  if (cat === 'event') {
    ...
    const record = await SDK.entities.Event.create({
      title: extracted.title || transcript.substring(0, 80),
      description: extracted.description || transcript,
  ...
  if (cat === 'finance') {
    const record = await SDK.entities.Expense.create({
  ...
  if (cat === 'travel') {
    const record = await SDK.entities.Holiday.create({
  ...
  if (cat === 'journal') {
    const record = await SDK.entities.Reflection.create({
      date: format(new Date(), 'yyyy-MM-dd'),
      content: transcript,
...
export default function VoiceCaptureHub({ onClose }) {
  ...
  const processAudio = async (blob) => {
    ...
    try {
      ...
      const { file_url } = await SDK.integrations.Core.UploadFile({ file: blob });

      // Transcribe + extract in one LLM call using vision+audio
      const transcribeResult = await SDK.integrations.Core.InvokeLLM({
