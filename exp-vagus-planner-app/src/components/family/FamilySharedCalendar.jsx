...
export default function FamilySharedCalendar() {
  ...
  const { data: familyEvents = [] } = useQuery({
    ...
    queryFn: async () => {
      const events = await SDK.entities.Event.filter({ category: 'family' });
      return events;
    }
  });
  ...
  const { data: familyTasks = [] } = useQuery({
    ...
    queryFn: async () => {
      const tasks = await SDK.entities.Task.filter({ category: 'family' });
      return tasks;
    }
  });
  ...
  const inviteMutation = useMutation({
    mutationFn: async (email) => {
      await SDK.users.inviteUser(email, 'user');
    },
    onSuccess: () => {
  ...
  const assignTaskMutation = useMutation({
    mutationFn: async ({ taskId, memberEmail }) => {
      ...
      await SDK.entities.Task.update(taskId, {
        ...task,
