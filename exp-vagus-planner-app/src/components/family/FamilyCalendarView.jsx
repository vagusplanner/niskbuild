...
export default function FamilyCalendarView({ groupId, user, memberEmails = [] }) {
  ...
  const { data: events = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.FamilyCalendarEvent.filter({ family_group_id: groupId }),
    enabled: !!groupId,
  });
  ...
  const { data: prayerLogs = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.PrayerLog.list('-date', 90),
  });

  const addEvent = useMutation({
    mutationFn: (data) => SDK.entities.FamilyCalendarEvent.create({ ...data, family_group_id: groupId, owner_email: user?.email, owner_name: user?.full_name || user?.email }),
    onSuccess: () => { qc.invalidateQueries(['familyEvents', groupId]); toast.success('Event added!'); },
  });

  const deleteEvent = useMutation({
    mutationFn: (id) => SDK.entities.FamilyCalendarEvent.delete(id),
