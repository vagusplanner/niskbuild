...
export default function FamilyPrayerGoals({ group, user }) {
  ...
  const { data: goals = [], isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => SDK.entities.FamilyPrayerGoal.filter({ family_group_id: group.id }),
  });

  const createMutation = useMutation({
    mutationFn: () => SDK.entities.FamilyPrayerGoal.create({
      ...form,
  ...
  const logProgressMutation = useMutation({
    mutationFn: async (goal) => {
      ...
      return SDK.entities.FamilyPrayerGoal.update(goal.id, {
