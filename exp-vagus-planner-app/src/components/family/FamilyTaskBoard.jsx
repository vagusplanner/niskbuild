...
export default function FamilyTaskBoard({ groupId, user, memberEmails = [], memberNames = [] }) {
  ...
  const { data: tasks = [], isLoading } = useQuery({
    ...
    queryFn: () => SDK.entities.FamilyTask.filter({ family_group_id: groupId }),
    enabled: !!groupId,
  });

  const createTask = useMutation({
    mutationFn: (data) => SDK.entities.FamilyTask.create(data),
    onSuccess: (created) => {
  ...
  const updateTask = useMutation({
    mutationFn: ({ id, data }) => SDK.entities.FamilyTask.update(id, data),
  ...
  const deleteTask = useMutation({
    mutationFn: (id) => SDK.entities.FamilyTask.delete(id),
