...
export default function SuperAgent({ isOpen, onClose }) {
  ...
  const { data: events = [] } = useQuery({
    queryKey: ['events'],
    queryFn: () => SDK.entities.Event.list('-start_date', 10),
    enabled: isOpen
  });

  const { data: tasks = [] } = useQuery({
    queryKey: ['tasks'],
    queryFn: () => SDK.entities.Task.list('-created_date', 20),
    enabled: isOpen
  });

  const { data: goals = [] } = useQuery({
    queryKey: ['goals'],
    queryFn: () => SDK.entities.Goal.list('-created_date', 10),
    enabled: isOpen
  });
...
Respond in 2-3 sentences max unless the user asks for detail.`;
      ...
      const response = await SDK.integrations.Core.InvokeLLM({ prompt });
