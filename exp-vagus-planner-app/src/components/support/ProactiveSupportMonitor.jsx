...
export default function ProactiveSupportMonitor() {
  ...
  const { data: recentActivity } = useQuery({
    ...
    queryFn: async () => {
      const [tasks, events, prayerLogs, settings] = await Promise.all([
        SDK.entities.Task.list('-created_date', 1),
        SDK.entities.Event.list('-created_date', 1),
        SDK.entities.PrayerLog.list('-date', 1),
  ...
  const submitFeedback = useMutation({
    mutationFn: async (feedbackData) => {
      return await SDK.entities.SupportTicket.create({
