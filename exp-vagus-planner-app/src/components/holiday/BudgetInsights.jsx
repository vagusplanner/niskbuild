  ...
  const { data: allExpenses = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const h of allHolidays) {
        const holidayExpenses = await SDK.entities.HolidayExpense.filter({ holiday_id: h.id });
    ...
    try {
      ...
      const pastExpenseData = await Promise.all(
        pastHolidays.map(async (h) => {
          const expenses = await SDK.entities.HolidayExpense.filter({ holiday_id: h.id });
          return {
      ...
      const { data } = await SDK.functions.invoke('generateBudgetInsights', {
        current_holiday: {
