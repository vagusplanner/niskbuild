...
export default function IslamicLifeTimeline() {
  ...
  const { data: milestones = [], isLoading } = useQuery({
    ...
    queryFn: () => SDK.entities.IslamicEvent.filter({ category: 'personal' }, 'date', 100)
      .then(r => r.filter(e => MILESTONE_TYPES.some(t => e.milestone_type === t.value || e.notes?.includes('"timeline":true'))))
      .catch(() => []),
  });

  const { data: prayerLogs = [] } = useQuery({ queryKey: ['prayerLogs'], queryFn: () => SDK.entities.PrayerLog.list('-date', 100) });
  const { data: quranMemory = [] } = useQuery({ queryKey: ['quranMemorization'], queryFn: () => SDK.entities.QuranMemorization.list('-created_date', 50).catch(() => []) });
  const { data: hajjFeedback = [] } = useQuery({ queryKey: ['hajjFeedback'], queryFn: () => SDK.entities.HajjFeedback.list('-created_date', 5).catch(() => []) });
