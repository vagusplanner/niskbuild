...
export default function PrayerStreakBadges() {
  ...
  const { data: prayerLogs = [] } = useQuery({
    queryKey: ['prayerLogs'],
    queryFn: () => SDK.entities.PrayerLog.list('-date', 400),
  });
  const { data: quranReadings = [] } = useQuery({
    queryKey: ['quranReadings'],
    queryFn: () => SDK.entities.QuranReading.list('-date', 100).catch(() => []),
  });
  const { data: userBadges = [] } = useQuery({
    queryKey: ['userAchievements'],
    queryFn: () => SDK.entities.UserAchievement.list('-created_date', 100).catch(() => []),
  });
  ...
  const mint = async (badge) => {
    ...
    setMinting(true);
    await SDK.entities.UserAchievement.create({
    ...
    await SDK.entities.GamificationPoints.create({
