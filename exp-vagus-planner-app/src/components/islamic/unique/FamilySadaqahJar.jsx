...
export default function FamilySadaqahJar() {
  ...
  const { data: families = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.FamilyGroup.filter({ member_emails: { $contains: '' } }, '-created_date', 1).catch(() => []),
  });
  ...
  const { data: contributions = [], isLoading } = useQuery({
    ...
    queryFn: () => family
      ? SDK.entities.FamilyContribution.filter({ family_group_id: family.id, type: 'sadaqah' }, '-date', 50)
      : SDK.entities.CharityDonation.list('-date', 30),
    enabled: true,
  });
  ...
  const contribute = async () => {
    ...
    try {
      ...
      if (family) {
        await SDK.entities.FamilyContribution.create({
          ...payload,
        ...
        await SDK.entities.CharityDonation.create(payload);
      }
