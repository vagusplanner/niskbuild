...
export default function GroupExpenseTracker({ group, members = [] }) {
  ...
  const { data: expenses = [] } = useQuery({
    ...
    queryFn: async () => {
      const allExpenses = await SDK.entities.HajjGroupExpense.list();
  ...
  const createExpenseMutation = useMutation({
    mutationFn: (data) => {
      ...
      return SDK.entities.HajjGroupExpense.create({
        ...data,
        group_id: group.id,
  ...
  const deleteExpenseMutation = useMutation({
    mutationFn: (id) => SDK.entities.HajjGroupExpense.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['groupExpenses', group.id] });
