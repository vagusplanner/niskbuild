...
export default function RecurringSadaqah() {
  ...
  const { data: activeSubs = [], isLoading: subsLoading } = useQuery({
    ...
    queryFn: async () => {
      const all = await SDK.entities.CharityDonation.filter({
  ...
  useEffect(() => {
    ...
    if (params.get('sadaqah_success') === 'true') {
      ...
      if (sessionId && user?.email && selectedCharity) {
        SDK.entities.CharityDonation.create({
  ...
  const cancelMutation = useMutation({
    mutationFn: async ({ subscription_id, record_id }) => {
      const { data } = await SDK.functions.invoke('cancelSadaqahSubscription', {
        subscription_id,
        record_id,
      });
  ...
  const handleSetup = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await SDK.functions.invoke('createSadaqahSubscription', {
