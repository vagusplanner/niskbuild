...
export default function RamadanFinanceLink() {
  ...
  const { data: expenses = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.Expense.filter({ date: { $gte: monthStart, $lte: monthEnd } }, '-date', 200),
    staleTime: 30000,
  });

  const { data: ramadanGoals = [] } = useQuery({
    queryKey: ['ramadanGoals'],
    queryFn: () => SDK.entities.RamadanGoal.list('-created_date', 10),
    staleTime: 30000,
  });

  const { data: fastingRecords = [] } = useQuery({
    queryKey: ['fastingRecordsMonth', monthStart],
    queryFn: () => SDK.entities.FastingRecord.filter({ date: { $gte: monthStart, $lte: monthEnd } }),
    staleTime: 30000,
  });
