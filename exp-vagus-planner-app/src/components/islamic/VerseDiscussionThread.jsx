...
export default function VerseDiscussionThread({ surahNumber, verseNumber, verseText }) {
  ...
  const { data: comments = [], isLoading } = useQuery({
    ...
    queryFn: () => SDK.entities.Comment.filter({
  ...
  useEffect(() => {
    const unsubscribe = SDK.entities.Comment.subscribe((event) => {
  ...
  const createCommentMutation = useMutation({
    mutationFn: (commentData) => SDK.entities.Comment.create(commentData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['verseComments', entityId] });
      setNewComment('');
      setReplyTo(null);
