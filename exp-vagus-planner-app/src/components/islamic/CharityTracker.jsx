...
export default function CharityTracker() {
  ...
  const { data: donations = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.CharityDonation.list()
  });

  const addDonationMutation = useMutation({
    mutationFn: (donation) => SDK.entities.CharityDonation.create(donation),
    onSuccess: () => {
  ...
  const createGoalMutation = useMutation({
    mutationFn: (goal) => SDK.entities.Goal.create(goal),
    onSuccess: () => {
  ...
  const handleSetReminder = () => {
    ...
    SDK.entities.UserSettings.list().then(settings => {
      const existing = settings[0];
      const reminderKey = `charity_reminder_${reminderFrequency}`;
      if (existing) {
        SDK.entities.UserSettings.update(existing.id, { [reminderKey]: true });
      }
    });
