...
export default function QuranLearningPlan() {
  ...
  const { data: goals = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.Goal.filter({ category: 'spiritual' })
  });
  ...
  const createPlanMutation = useMutation({
    mutationFn: async ({ surah, vpd }) => {
      ...
      return SDK.entities.Goal.create({
  ...
  const toggleStepMutation = useMutation({
    mutationFn: ({ goal, stepIndex }) => {
      ...
      return SDK.entities.Goal.update(goal.id, { action_steps: steps, progress, status: progress === 100 ? 'completed' : 'in_progress' });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['quranLearningGoals'] });
    }
