...
export default function SpiritualHabitTracker() {
  ...
  const { data: habits = [], isLoading } = useQuery({
    ...
    queryFn: () => SDK.entities.Habit.filter({ category: 'spiritual' }, 'created_date', 50),
  });
  ...
  const toggleDay = async (habit, date) => {
    ...
    await SDK.entities.Habit.update(habit.id, { completion_dates: next });
    queryClient.invalidateQueries({ queryKey: ['spiritualHabits'] });
  };

  const removeHabit = async (habit) => {
    await SDK.entities.Habit.delete(habit.id);
    queryClient.invalidateQueries({ queryKey: ['spiritualHabits'] });
