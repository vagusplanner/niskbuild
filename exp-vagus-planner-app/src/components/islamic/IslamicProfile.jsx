...
export default function IslamicProfile() {
  ...
  const { data: quranBookmarks = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.QuranVerse.filter({ is_favorite: true }, '-created_date', 100)
  });
  ...
  const deleteHadithMutation = useMutation({
    mutationFn: (id) => SDK.entities.Hadith.delete(id),
    onSuccess: () => {
  ...
  const deleteQuranBookmarkMutation = useMutation({
    mutationFn: (id) => SDK.entities.QuranVerse.delete(id),
    onSuccess: () => {
  ...
  const toggleHadithFavoriteMutation = useMutation({
    mutationFn: ({ id, isFavorite }) => 
      SDK.entities.Hadith.update(id, { is_favorite: !isFavorite }),
    onSuccess: () => {
      queryClient.invalidateQueries(['user-hadiths']);
    }
  });
  ...
  const handleDeleteAccount = async () => {
    setDeleting(true);
    try {
      await SDK.functions.invoke('deleteUserAccount', {});
