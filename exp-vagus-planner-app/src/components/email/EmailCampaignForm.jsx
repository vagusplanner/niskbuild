...
export default function EmailCampaignForm({ campaign, onClose, onSave }) {
  ...
  const handleGenerateContent = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await SDK.functions.invoke('draftEmailCampaign', {
        goal: formData.goal,
  ...
  const handleSave = async () => {
    ...
    try {
      if (campaign) {
        await SDK.entities.EmailCampaign.update(campaign.id, formData);
        toast.success('Campaign updated');
      } else {
        await SDK.entities.EmailCampaign.create(formData);
