...
export default function WeeklyEmailDigest() {
  ...
  const toggleMutation = useMutation({
    mutationFn: async (enabled) => {
      if (settings?.id) {
        return SDK.entities.UserSettings.update(settings.id, { journal_reminder_enabled: enabled });
      }
      return SDK.entities.UserSettings.create({ journal_reminder_enabled: enabled });
    },
    onSuccess: () => {
      refetch();
  ...
  const sendNow = async () => {
    ...
    setSending(true);
    try {
      await SDK.functions.invoke('generateWeeklyDigest', { email: user.email });
