...
function WeightLogger({ goal, onLogged }) {
  ...
  const handleLog = async () => {
    ...
    await SDK.entities.FitnessGoal.update(goal.id, {
...
function AdherenceLogger({ goal, onLogged }) {
  ...
  const handleLog = async () => {
    ...
    await SDK.entities.FitnessGoal.update(goal.id, { meal_adherence_log: updatedLog });
    setNotes('');
...
export default function FitnessProgressDashboard({ goal, onRefresh }) {
  ...
  const { data: exercises = [] } = useQuery({
    queryKey: ['exercises'],
    queryFn: () => SDK.entities.Exercise.list('-date', 20),
  });

  const handleRefreshMacros = async () => {
    setMacroLoading(true);
    try {
      const res = await SDK.functions.invoke('adjustMacrosForFitnessGoal', { fitness_goal_id: goal.id });
