...
export default function PilgrimageConcierge({ currentLocation = 'Mecca', userInterests = [] }) {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const { data } = await SDK.functions.invoke('pilgrimageConcierge', {
        question: text,
  ...
  const handleGetRecommendations = async () => {
    setLoading(true);
    try {
      const { data } = await SDK.functions.invoke('generateLocalRecommendations', {
        current_location: currentLocation,
  ...
  const handleBookingRequest = async (text) => {
    setLoading(true);
    try {
      const { data } = await SDK.functions.invoke('assistBooking', {
