...
export default function AIGoalsManager({ isOpen, onClose }) {
  ...
  const { data: goals = [], isLoading } = useQuery({
    queryKey: ['aiGoals'],
    queryFn: () => SDK.entities.AIPersonalization.list('-priority')
  });

  const { data: analysis, refetch: refetchAnalysis } = useQuery({
    queryKey: ['userAnalysis'],
    queryFn: async () => {
      const res = await SDK.functions.invoke('analyzeUserPersonalization', {});
      return res.data;
    },
    enabled: isOpen
  });
  ...
  const createGoalMutation = useMutation({
    mutationFn: (data) => SDK.entities.AIPersonalization.create(data),
    onSuccess: () => {
  ...
  const toggleGoalMutation = useMutation({
    mutationFn: ({ id, is_active }) => 
      SDK.entities.AIPersonalization.update(id, { is_active }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['aiGoals'] });
    }
  });
  ...
  const deleteGoalMutation = useMutation({
    mutationFn: (id) => SDK.entities.AIPersonalization.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['aiGoals'] });
