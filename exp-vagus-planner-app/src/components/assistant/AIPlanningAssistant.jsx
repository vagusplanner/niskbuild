...
function GoalBreakdownMode({ goals, onTasksCreated }) {
  ...
  const createAll = async () => {
    ...
    try {
      ...
      await SDK.entities.Task.bulkCreate(tasks);
  ...
  const createEvents = async () => {
    ...
    try {
      for (const ev of result.events) {
        await SDK.entities.Event.create({ ...ev, source: 'app' });
      }
...
function ActivityFeedMode() {
    ...
    const load = async () => {
      try {
        const [events, tasks, goals] = await Promise.all([
          ...
          SDK.entities.Goal.list('-updated_date', 5),
        ]);
        const items = [
  ...
  const accept = async (s, i) => {
    ...
    try {
      ...
      if (s.type === 'task') {
        await SDK.entities.Task.create({ title: s.title, description: s.description, due_date: dueDate, status: 'todo', priority: 'medium' });
        queryClient.invalidateQueries({ queryKey: ['upcomingTasks'] });
      } else {
        const start = addDays(new Date(), s.due_days || 1);
        start.setHours(10, 0, 0, 0);
        const end = new Date(start.getTime() + 60 * 60000);
        await SDK.entities.Event.create({ title: s.title, description: s.description, start_date: start.toISOString(), end_date: end.toISOString(), category: 'personal', source: 'app' });
        queryClient.invalidateQueries({ queryKey: ['events'] });
      }
  ...
  const createTasks = async () => {
    ...
    try {
      ...
      if (tasksToCreate.length > 0) {
        await SDK.entities.Task.bulkCreate(tasksToCreate);
        queryClient.invalidateQueries({ queryKey: ['upcomingTasks'] });
...
export default function AIPlanningAssistant({ isOpen, onClose, period = 'week' }) {
  ...
  const { data: tasks = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.Task.filter({ status: { $in: ['todo', 'in_progress'] } }, '-due_date', 20),
    enabled: !!user
  });
  const { data: events = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return SDK.entities.Event.filter({ start_date: { $gte: today.toISOString(), $lte: ahead.toISOString() } });
    },
    enabled: !!user
  });
  const { data: goals = [] } = useQuery({
    queryKey: ['userGoals'],
    queryFn: () => SDK.entities.Goal.filter({ status: { $in: ['not_started', 'in_progress'] } }, '-created_date', 20),
    enabled: !!user
  });
