...
export default function ExerciseTracker() {
  ...
  const { data: todayExercises = [] } = useQuery({
    ...
    queryFn: () => SDK.entities.Exercise.filter({ date: formData.date }, '-created_date')
  });
  ...
  const { data: weekExercises = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const all = await SDK.entities.Exercise.list('-date', 100);
      return all.filter(e => e.date >= weekStart);
    }
  });

  const createMutation = useMutation({
    mutationFn: (data) => SDK.entities.Exercise.create(data),
    onSuccess: () => {
