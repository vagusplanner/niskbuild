...
export default function ChallengesList() {
  ...
  const { data: userChallenges = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const challenges = await SDK.entities.UserChallenge.filter({ 
      ...
      const withDetails = await Promise.all(
        challenges.map(async (uc) => {
          const [challenge] = await SDK.entities.Challenge.filter({ id: uc.challenge_id });
          return { ...uc, challenge };
        })
      );
  ...
  const generateChallengesMutation = useMutation({
    mutationFn: () => SDK.functions.invoke('generatePersonalizedChallenges', {}),
    onSuccess: () => {
  ...
  const completeChallengeMutation = useMutation({
    mutationFn: async (userChallenge) => {
      ...
      await SDK.entities.UserChallenge.update(userChallenge.id, {
        status: 'completed',
        progress: 100,
      ...
      // Award points
      await SDK.functions.invoke('awardPoints', {
